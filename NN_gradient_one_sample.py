...
def partial_derivatives(x, y, W, V, b, c):
    ...
    l = c + V @ sig(b + W @ x)
    dLdf = np.exp(l) / np.sum(np.exp(l)) -np.array([[0,0,1,0]]).T   
    dLdc = dLdf     #np.ones(c.shape)
    dLdV = dLdc * sig(b + W @ x).T       #np.ones(V.shape)
    dLdb = sigp(b + W @ x) * (V.T @ dLdf)     #np.ones(b.shape)
    dLdW = dLdb * x.T              #np.ones(W.shape)
